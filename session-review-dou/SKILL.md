...
1. 检测 jq 是否可用（`command -v jq`）。无则提示：`jq 未安装，scan.py 将使用 Python stdlib。安装 jq 可加速：brew install jq / apt install jq`
2. 运行 `python3 scripts/scan.py --list` 展示会话列表
3. 让用户选择至多 3 个会话（通过编号）
