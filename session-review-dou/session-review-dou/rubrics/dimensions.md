...
### 四问定位法
对每个会话逐条问：
1. AI 第一次回复就走偏了方向？→ **意图对齐瓶颈**
2. AI 说"修好了"但实际没修好？→ **完成验证瓶颈**
3. 同样的错误修过一次，又犯了？→ **错误固化瓶颈**
4. 你重复手动做同样的准备工作？→ **循环升级机会**

回答"是"的那条就是该会话的 Herbie。

### 跨会话瓶颈排名
多个会话时，统计每个瓶颈出现频次，按估计时间浪费排序：
- 同一命令模板跨 ≥2 个会话出现 → 🔴 跨会话重复模式
- 同会话内重复 ≥5 次 → 🟡 会话内重复模式
- 已解决（已写规则/skill）→ 🟢 已固化
...
| 指标 | 阈值 | 含义 |
|------|------|------|
| 重复指令率 | ≥15% 用户消息重复 | 该封装为 /goal |
| 命令模板率 | ≥5 次相同 CLI 命令 | 该升级为 /loop |
| 纠正往返率 | ≥20% 交互轮含纠正 | 需要反向复述检查点 |
| 跨会话重复 | ≥2 个会话含相似任务 | 该封装为 skill 或 routine |
