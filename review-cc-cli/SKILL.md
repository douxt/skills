...
| `/review-cc-cli --timeout <秒>` | 控制 | 子进程超时（默认 300 = 5 分钟），超时后重试一次 |
...
| 顺序 | 维度 | Rubric | 阻断 | 关注点 |
|------|------|--------|------|--------|
| ① | correctness | correctness.md | ✅ | 逻辑、边界、空值、类型、异步、状态、并发 |
| ② | security | security.md | ✅ | OWASP top 10、注入、认证、密钥 |
| ③ | performance | performance.md | ⚠️ | N+1、内存、阻塞 I/O、算法复杂度 |
...
/review-cc-cli --parallel [维度列表] <范围>
        │
  ① 确定激活维度（全默认 4 维，或用户指定子集）
        │
  ② 并行启动 N 个 claude -p（Bash 后台任务，run_in_background: true）
     ┌────────┬────────┬────────┬────────┐
     │correct │security│  perf  │ style  │
     └───┬────┴───┬────┴───┬────┴───┬────┘
         │        │        │        │
  ③ TaskOutput 收集所有 agent 的 JSON 输出
         │
  ④ 主进程去重合并（三级管道）：
     ...
     Stage 3: 候选对送入 verifier 判「同一问题不同表象」→ 合并+交叉引用
         │
  ⑤ 启动 verifier agent（claude -p），逐条验证：
     - ≤15 条 → 全验；>15 条 → 按 severity 排序截断
     - 输出 confirmed / false_positive / uncertain
     - verifier 不生成新发现，遗漏标注「⚠️ Verifier 提示」
         │
  ⑥ 合并报告：
     ✅ Confirmed（N 条）
     ⚠️ Uncertain（N 条）
...
- 每轮并行启动 N 个 agent + 1 个 verifier
- 收敛判断按全局：所有维度合计连续 3 轮无新确认 → 停止
- 阻断级维度（correctness/security）失败 → 本轮不计，重试一次
...
| 场景 | 处理 |
|------|------|
| 某维度 agent 超时/被拦截 | 阻断级（correctness/security）→ 本轮不计，重试；非阻断级 → 标注缺失 |
...
| 子进程超时（默认 300s，可通过 `--timeout` 调整） | 重试一次，再失败则提示用户手动检查 |
